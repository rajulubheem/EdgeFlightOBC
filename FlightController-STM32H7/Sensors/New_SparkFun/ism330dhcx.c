...
int32_t ISM330DHCX_Init(ISM330DHCX_Object_t *pObj)
{
  ...
  if (ism330dhcx_xl_full_scale_set(&(pObj->Ctx), ISM330DHCX_4g) != ISM330DHCX_OK)
  {
    return ISM330DHCX_ERROR;
  }
  ...
  /* Full scale selection. */
  if (ism330dhcx_gy_full_scale_set(&(pObj->Ctx), ISM330DHCX_500dps) != ISM330DHCX_OK)
  {
    return ISM330DHCX_ERROR;
  }
    // Turn on the accelerometer's filter and apply settings
    ism330dhcx_xl_filter_lp2_set(&(pObj->Ctx), ISM330DHCX_LP_ODR_DIV_100);  // Enable LP2 filter for accelerometer
    // Turn on the gyroscope's filter and apply settings
    ism330dhcx_gy_filter_lp1_set(&(pObj->Ctx), ISM330DHCX_MEDIUM);  // Enable LP1 filter for gyroscope
